...
//			NgMaterial.SetMaterialColor(tarMat, m_CopyColor);
	...
	public Object OverwriteClipboardObject(GameObject tarGameObject, Object tarComponent, int tarIndex)
	{
		if (IsGameObject())
		{
			...
			newObj.transform.parent = tarGameObject.transform; // ｿﾀｷ?ﾀﾖｴﾙ... ｻ鄙?ﾈﾇﾏｹﾇｷﾎ ｳｪﾁﾟｿ｡ ﾃｳｸｮ
	...
	public static Component ObjectCopy(Component src, Transform dstSelTrans, Object dstSelObj, bool bCreateNewTarget, bool bSearchTarComponent, bool bCopyAllTarComs)
	{
		...
		// bCreateNewTarget ﾂ?ﾀﾌｿｩｵｵ, bSearchTarComponent ﾂ?ﾀﾌｸ?ｻ?ｷﾎ ｻ?ｼｺｾﾈﾇﾔ

		if ((bCreateNewTarget == false) && src.GetType() == dstSelObj.GetType())	// ｽﾅｱﾔｻ?ｼｺ ﾁｶｰﾇﾀﾌ ｾﾆｴﾏｸ? ｰｰﾀｺｰﾔｿ｡ ｵ蠍?ﾁﾟ
		{
// 			if (NgAssembly.IsValidCopy(dst, src))
		...
		if (tarComs == null || tarComs.Length <= 0)					// ｾ?ｴﾙ ｸｸｵ鯊ﾚ
		{
			tarComs		= new Component[1];
		...
		if (tarComs != null)
		{
			foreach (Component tarcom in tarComs)					// ｳｻｿ?ｺｹｻ?
			{
				if (src != tarcom)
				{
